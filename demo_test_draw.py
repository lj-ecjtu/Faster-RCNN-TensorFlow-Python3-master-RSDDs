...
def load_image_set_index():
    ...
    image_set_file = os.path.join(cf.FLAGS2["root_dir"],'DateSet','RSDDs_dataset_LJ','RSDDsTest','VOCDevkit2007',"VOC2007",'ImageSets','Main','test.txt')
...
def image_path_from_index(index):
    ...
    """
    image_path = os.path.join(cf.FLAGS2["root_dir"],'DateSet','RSDDs_dataset_LJ','RSDDsTest','VOCdevkit2007','VOC2007','JPEGImages',
                                  index + ".jpg")
    assert os.path.exists(image_path), \
            'Path does not exist: {}'.format(image_path)
    return image_path    

def vis_detections(im, class_name, dets,image_name,outputPath, thresh=0.5):
    ...
    plt.tight_layout()
    plt.draw()
    image_name=os.path.join(outputPath,image_name+'.jpg')
...
if __name__ == '__main__':
    ...
    tfmodel = os.path.join(cf.FLAGS2["root_dir"], 'Module', 'LJModule', 'Faster RCNN', demonet, 'RSDDs_1', 'default', NETS[demonet][0])
    ...
    output_path='G:\DeepLearning\Project\LJProject\Faster-RCNN\Faster-RCNN-TensorFlow-Python3-master-RSDDs\demo_output\\vgg16\RSDDs_train_test'
