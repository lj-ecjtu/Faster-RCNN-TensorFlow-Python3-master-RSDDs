...
def voc_eval(detpath,
    ...
    """
    splitlines = [x.strip().split('\t') for x in lines]    #读处的每一行以空格分隔开，保存为一个列表
...
def _do_python_eval(res_prefix,i,output_dir = 'output'):
    _root_dic=os.path.abspath(os.path.join(os.path.dirname(__file__), '..','..','..','..','..'))   #f://DeepLearning
    _devkit_path = os.path.join(_root_dic,'DateSet','NEUDet_LJ','NEUTrain','VOCdevkit2007','VOC2007')
    _year = '2007'
    _classes = ('__background__','crazing', 'inclusion', 'patches', 'pitted_surface', 'rolled-in_scale', 'scratches') 
    #测试结果地址
    res_prefix = os.path.join(res_prefix, 'test_result','NEU_5_train_train_nms','NEU_train_train_{:s}_0.001_0.0001','comp3_det_test_')
    res_prefix=res_prefix.format(i)
    ...
    use_07_metric = False
    print()
    print('test_result: NEU_5_train_test_nms__iter_{}'.format(i))
    '''
    ...
    for i, cls in enumerate(_classes):
        ...
        print('{} = {:.4f}'.format(cls, ap),end='\t')
        # 将单个类别的计算结果保存到本地
        with open(os.path.join(output_dir, cls + '_pr.pkl'), 'wb') as f:
            pickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)    # pkl以字典的形式保存
    print()
    print('Faster-RCNN:  mAP = {:.4f}'.format(np.mean(aps)))
    print()
    print('~~~~~~~~')
    print('~~~~~~~~')
...
if __name__ == '__main__':
    ...
    '''
    iters=["70000","80000","90000","100000","110000","120000"]
    for i in iters:
        output_path = 'mAP\\AP_result\\NEU_5_train_train_nms\\NEU_train_train_{:s}_0.001_0.0001'
        output_path=output_path.format(i)
        if not os.path.isdir(output_path):
            os.mkdir(output_path)
        _do_python_eval(args.path,i, output_dir =output_path)
