...
tf.app.flags.DEFINE_integer('step_size_1', 30000, "Step size for reducing the learning rate, currently only support one step")
tf.app.flags.DEFINE_integer('step_size_2', 100000, "Step size for reducing the learning rate, currently only support one step")
...
#(600,)为包含一个对象的元组，这里的逗号不能少
FLAGS2["scales"] = (600,)
print(len(FLAGS2["scales"]))  #输出为1
...
# Testing Parameters #
######################
tf.app.flags.DEFINE_string('test_mode', "nms", "Test mode for bbox proposal")  # nms, top
...
pretrained_model=os.path.join(FLAGS2["root_dir"],'Module','LJModule')
tf.app.flags.DEFINE_string('pretrained_model', pretrained_model+"\\vgg16.ckpt", "Pretrained network weights")

print(FLAGS2["root_dir"])

#加载数据的路径
FLAGS2["data_dir"] = osp.abspath(osp.join(FLAGS2["root_dir"],'DateSet','RSDDs_dataset_LJ','RSDDsTrain'))
...
def get_output_dir(imdb, weights_filename):
    ...
    (if not None).
    """
    outdir = osp.abspath(osp.join(FLAGS2["root_dir"],'Module','LJModule','Faster RCNN','vgg16', 'RSDDs_3'))    #修改路径时只修改最后一个字符串
